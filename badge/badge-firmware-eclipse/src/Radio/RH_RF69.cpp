...
bool RH_RF69::init() {
	...
	if (_myInterruptIndex == 0)
		//HERE:  in stm32f1xx_it.c is there irq - need to have that call isr0....
...
void RH_RF69::readFifo() {
	ATOMIC_BLOCK_START; digitalWrite(_slaveSelectPin, LOW);
	...
	} digitalWrite(_slaveSelectPin, HIGH); ATOMIC_BLOCK_END;
...
//DAC
	//opmode = spiRead(RH_RF69_REG_01_OPMODE);
...
void RH_RF69::setEncryptionKey(uint8_t* key) {
	if (key) {
		...
		spiWrite(RH_RF69_REG_3D_PACKETCONFIG2,
				spiRead(
						RH_RF69_REG_3D_PACKETCONFIG2) | RH_RF69_PACKETCONFIG2_AESON);
	} else {
		spiWrite(RH_RF69_REG_3D_PACKETCONFIG2,
				spiRead(
						RH_RF69_REG_3D_PACKETCONFIG2) & ~RH_RF69_PACKETCONFIG2_AESON);
