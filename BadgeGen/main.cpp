...
static void setUpIV(const char *productName) {
    ...
    sha.CalculateDigest(&prep[0],(const byte *)productName, strlen(productName));
    memcpy(&iv[0],&prep[0],CryptoPP::AES::BLOCKSIZE);
}

static void GenerateKey(byte key[CryptoPP::SHA::DIGESTSIZE], const std::string &password) {
    CryptoPP::SHA sha;
    sha.CalculateDigest(&key[0],(const byte *)password.c_str(), password.length());
}

void createDaemonKeyPair() {

}

static bool makeKeyPair(const char *password, const std::string &privateKeyFileName, const std::string &publicKeyFileName) {
    ...
    if(privateKey.Validate(random_pool,3)) {
        ...
        if(publicKey.Validate(random_pool,3)) {
            DEBUGMSG("Point on Curve 'X':= " << std::hex << publicKey.GetPublicElement().x << std::dec);
            DEBUGMSG("Point on Curve 'Y':= " << std::hex << publicKey.GetPublicElement().y << std::dec);
            ...
            CryptoPP::Base64Encoder* base64_enc = new CryptoPP::Base64Encoder(new FileSink(privateKeyFileName.c_str()));
            CryptoPP::CBC_Mode<CryptoPP::AES>::Encryption aes(key,CryptoPP::AES::BLOCKSIZE, iv);
            CryptoPP::StreamTransformationFilter aes_enc(aes, base64_enc);
            aes_enc.Put((const byte *)privateKeyString.c_str(),privateKeyString.length());
            aes_enc.MessageEnd();
            publicKey.Save(CryptoPP::Base64Encoder(new FileSink(publicKeyFileName.c_str())).Ref());
            INFOMSG("Key pair successfully made: private key file is encrypted with " << aes.StaticAlgorithmName().c_str());
...
static bool readInPrivateKey(const std::string &password, PrivateECCKeyType &PrivateKey, const std::string &privateKeyFileName) {
    bool bRetVal = false;
    if(exists(privateKeyFileName.c_str())) {
        //need to check to insure file exists
        std::string cipher;
        CryptoPP::Base64Decoder* base_dec = new CryptoPP::Base64Decoder(new CryptoPP::StringSink(cipher));
        CryptoPP::FileSource file(privateKeyFileName.c_str(),true,base_dec,false);
        byte key[CryptoPP::SHA::DIGESTSIZE];
        GenerateKey(key,password);
        assert(CryptoPP::SHA::DIGESTSIZE>=CryptoPP::AES::BLOCKSIZE);
        ...
        CryptoPP::StreamTransformationFilter stfDecryptor(cbcDecryption, new CryptoPP::StringSink( plantextPrivateKey ) );
        stfDecryptor.Put( reinterpret_cast<const unsigned char*>( cipher.data() ), cipher.size() );
        stfDecryptor.MessageEnd();
        CryptoPP::StringSource tprivateKeySink((const byte *)plantextPrivateKey.data(),plantextPrivateKey.length(),true);
        PrivateKey.Load(tprivateKeySink);
        DEBUGMSG("private Exponent:= " << PrivateKey.GetPrivateExponent());
        bRetVal = true;
    } else {
        WARNMSG("****************************************************************************************");
        WARNMSG("Can not open private key file:  " << privateKeyFileName.c_str());
        WARNMSG("****************************************************************************************");
    }    
    return bRetVal;
}

static bool readInPublicKey(const std::string &publicKeyFileName, PubECCKeyType &publicKey) {
    bool bRetVal = false;
    if(exists((publicKeyFileName.c_str()))) {
        //check if file exists
        std::string encoded;
        CryptoPP::Base64Decoder* base_dec = new CryptoPP::Base64Decoder(new CryptoPP::StringSink(encoded));
        CryptoPP::FileSource file(publicKeyFileName.c_str(),true,base_dec,false);
        CryptoPP::StringSource publicStringSink((const byte *)encoded.c_str(),encoded.length(),true);
...
int main(int argc, char *argv[]) {
    ...
    while ((ch = getopt(argc, argv, "cgn:")) != -1) {
        switch (ch) {
            case 'c':
                create =1;
            break;
